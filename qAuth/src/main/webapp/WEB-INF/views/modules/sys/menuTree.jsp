...
<body> --%>
	<div class="accordion" id="menu-${param.parentId}"><c:set var="menuList" value="${fns:getMenuList()}"/><c:set var="firstMenu" value="true"/><c:forEach items="${menuList}" var="menu" varStatus="idxStatus"><c:if test="${(menu.parent.id eq (not empty param.parentId ? param.parentId:1) && (menu.isShow eq '1' || ((fns:getUser() eq '1') && (menu.id eq '3') )))}">
		<div class="accordion-group">
		    ...
		    <div id="collapse-${menu.id}" class="accordion-body collapse ${not empty firstMenu && firstMenu ? 'in' : ''}">
				<div class="accordion-inner">
					<ul class="nav nav-list"><c:forEach items="${menuList}" var="menu2"><c:if test="${(menu2.parent.id eq menu.id && (menu2.isShow eq '1' || ((fns:getUser() eq '1') && (menu2.parent.id eq '3') )))}">
