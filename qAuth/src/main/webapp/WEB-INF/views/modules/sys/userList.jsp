...
<script type="text/javascript">
	...
	function restForm(){
		...
		$("#officeName").val(null);
		/* $("#position").val(null);
		$("#s2id_position").children().first().find("span").first().html(""); */
	}
...
<body>
	...
	<form:form id="searchForm" modelAttribute="user"
		...
		<sys:tableSort id="orderBy" name="orderBy" value="${page.orderBy}"
			callback="page();" />
		<ul class="ul-form" style="height: 40px;">
			<li><label>归属公司：</label>
		...
		<ul class="ul-form" style="padding-top: 8px;height: 40px;">
			<li><label>归属部门：</label>
			<sys:treeselect id="office" name="office.id"
					...
					allowClear="true" notAllowSelectParent="false" /></li>
			<li><label>姓&nbsp;&nbsp;&nbsp;名：</label>
			<form:input path="name" htmlEscape="false" maxlength="50"
					class="input-medium" /></li>
			<li class="btns">
			    <input id="btnSubmit" class="btn btn-primary" type="submit" value="查询" onclick="return page();" />
				<input id="btnSubmit" class="btn btn-primary" type="button" value="重置" onclick="restForm()" />  
				<input id="btnExport" class="btn btn-primary"  style="display:none" type="button" value="导出" /> 
				<input id="btnImport" class="btn btn-primary"  style="display:none" type="button" value="导入" /></li>
			<li class="clearfix"></li>
		</ul>
	</form:form>
		...
		<tbody>
			<c:forEach items="${page.list}" var="user">
				...
				<td>${user.roleNames}</td> --%>
					<shiro:hasPermission name="sys:user:edit">
						<td><a href="${ctx}/sys/user/form?id=${user.id}">修改</a>
						 <a
