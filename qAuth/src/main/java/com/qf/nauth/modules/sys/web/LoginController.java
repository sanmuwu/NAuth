...
import com.qf.nauth.common.utils.CookieUtils;
...
import com.qf.nauth.modules.sys.constants.AuthParamConstants;
...
import com.qf.nauth.modules.sys.security.realms.SystemDbAuthenticationRealm.DbPrincipal;
...
/*		if (Global.TRUE.equals(Global.getConfig("notAllowRefreshIndex"))) {
	...
	public String loginFail(HttpServletRequest request, HttpServletResponse response, Model model) throws IOException {
		...
		if(null != exceptionClsname && (AuthenticationException.class.isAssignableFrom(exceptionClsname)
			...
			model.addAttribute(AuthParamConstants.getUserNameParam(), username);
			...
			model.addAttribute(AuthcAuthenticationFilter.DEFAULT_ERROR_KEY_ATTRIBUTE_NAME, exceptionClsname.getName());
			model.addAttribute(AuthcAuthenticationFilter.MESSAGE_PARAM, message);
			
			if (logger.isDebugEnabled()){
				logger.debug("login fail, active session size: {}, message: {}, exception: {}", 
					sessionDAO.getActiveSessions(false).size(), message, exceptionClsname.getName());
			}
			
			// 验证失败清空验证码
			if (!UnauthorizedException.class.getName().equals(exceptionClsname.getName())) {
				// 非授权异常，登录失败，验证码加1。
