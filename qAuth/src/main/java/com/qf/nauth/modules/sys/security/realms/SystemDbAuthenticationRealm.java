...
import com.qf.nauth.common.utils.SpringContextHolder;
...
import com.qf.nauth.modules.sys.security.exception.DbAuthenticationException;
import com.qf.nauth.modules.sys.security.tokens.UsernamePasswordToken;
...
import com.qf.nauth.modules.sys.web.LoginController;
...
public class SystemDbAuthenticationRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken dbAuthcToken) {
		...
		if (user != null) {
			...
			throw new DbAuthenticationException("msg:用户名或密码错误, 请重试.");
		}
	}
	...
	protected void assertCredentialsMatch(AuthenticationToken token, AuthenticationInfo info) 
			throws DbAuthenticationException {
		//doing nothing, password authentication will delegate to LDAP realm
		
	/*	//保留一个系统用户用数据库验证
		UsernamePasswordToken userToken = (UsernamePasswordToken) token;
		//获取用户
		User user = getSystemService().getUserByLoginName(userToken.getUsername());
		
		if(user.getId()!= null && "1".equals(user.getId())){
